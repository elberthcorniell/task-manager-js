...
describe('Task class,', () => {
    ...
    describe('save', () => {
        test('saves the given item to localStorage', () => {
            const task = new Task({ description, title, dueDate, priority, group });
            task.save();
            expect(localStorage.getItem('tasks')).toEqual(JSON.stringify({ [group]: [{ title, description, dueDate, priority, group }] }))
        })
    })
